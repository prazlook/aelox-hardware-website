...
export const ASICStatusCard = ({ asic, maxTemp, onTogglePower, onToggleFan, onToggleOverclock, onPowerAction, triggerGlow }: ASICStatusCardProps) => {
    ...
    <>
      <div className="relative h-full">
        <div
          ref={cardRef}
          className={cn(
            "relative z-0 p-4 rounded-2xl border border-transparent flex flex-col space-y-3 transition-all duration-300 bg-theme-card h-full overflow-hidden",
            isOffline && !isBootingUp && "grayscale opacity-70"
          )}
        >
          {isGlowing && (
            <div className="absolute inset-0 z-0 animate-rainbow-glow bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-transparent via-transparent to-transparent" />
          )}
          <div className="relative z-10 flex flex-col space-y-3 h-full">
            ...
            <div className={cn("flex-grow", contentAnimationClass, { 'animate-boot-up-item': isBootingUp })} style={isBootingUp ? getBootAnimationStyles(1) : getAnimationStyles(1)}>
              <Button 
                className="w-full bg-theme-cyan text-black font-bold hover:bg-theme-cyan/90 rounded-xl mt-auto"
                disabled={isOffline}
              >
